...
class ProgressBarCallback(Callback):
    ...
    def on_training_step_end(
        ...
        # All-reduce loss across all ranks, then only rank0 displays
        if dist.is_available() and dist.is_initialized():
            loss_g = loss.detach().clone().cuda()
            dist.all_reduce(loss_g, op=dist.ReduceOp.AVG)
            if dist.get_rank() == 0:
                self.train_pbar.set_postfix(loss=f"{loss_g.item():.2f}")
                self.train_pbar.update()
        else:
            self.train_pbar.set_postfix(loss=f"{loss.item():.2f}")
            self.train_pbar.update()
