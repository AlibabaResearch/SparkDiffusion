...
WAN_REPO=${WAN_REPO:-pretrain_weights/Wan2.1-T2V-14B}
OUTPUT_ROOT=${OUTPUT_ROOT:-outputs/distill}
export IMAGINAIRE_OUTPUT_ROOT=${OUTPUT_ROOT} 
# teacher: native 14B dense DiT (sharded safetensors + index.json)
TEACHER_CKPT=${TEACHER_CKPT:-${WAN_REPO}}
# student: 14B RoLa sparse-finetuning checkpoint (Stage 1 output of the full framework)
...
torchrun --nproc_per_node=${NPROC_PER_NODE:-8} \
    ...
    experiment=wan2pt1_14B_res720p_t2v_rola_distill \
