...
WAN_REPO=${WAN_REPO:-pretrain_weights/Wan2.2-T2V-A14B}
OUTPUT_ROOT=${OUTPUT_ROOT:-outputs/distill}
export IMAGINAIRE_OUTPUT_ROOT=${OUTPUT_ROOT}
TEACHER_CKPT=${TEACHER_CKPT:-${WAN_REPO}/high_noise_model}
TEACHER_CKPT_LOW=${TEACHER_CKPT_LOW:-${WAN_REPO}/low_noise_model}
STUDENT_CKPT=${STUDENT_CKPT:-pretrain_weights/finetuned_sparse_high.pth}
STUDENT_CKPT_LOW=${STUDENT_CKPT_LOW:-pretrain_weights/finetuned_sparse_low.pth}
...
DATASET_ROOT=${DATASET_ROOT:-datasets/distill/Wan2.1_14B_720p_16:9_Euler-step100_shift-5.0_cfg-5.0_seed-0_250K}

require_dir "SLA_SRC" "${SLA_SRC:-}"
export SLA_SRC
require_path "TEACHER_CKPT" "${TEACHER_CKPT}"
require_path "TEACHER_CKPT_LOW" "${TEACHER_CKPT_LOW}"
require_path "STUDENT_CKPT" "${STUDENT_CKPT}"
require_path "STUDENT_CKPT_LOW" "${STUDENT_CKPT_LOW}"
require_file "VAE_PATH" "${VAE_PATH}"
require_file "T5_PATH" "${T5_PATH}"
...
torchrun --nproc_per_node=${NPROC_PER_NODE:-8} \
    ...
    experiment=wan2pt2_A14B_res720p_t2v_rola_distill_joint \
    model.config.teacher_ckpt="${TEACHER_CKPT}" \
    model.config.teacher_ckpt_low="${TEACHER_CKPT_LOW}" \
    model.config.student_ckpt="${STUDENT_CKPT}" \
    model.config.student_ckpt_low="${STUDENT_CKPT_LOW}" \
    ...
    model.config.joint_low_d_time_full_range=true
