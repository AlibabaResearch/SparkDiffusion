#!/bin/bash
# ============================================================================
# Wan2.1 T2V / I2V single-case distilled student evaluation.
#   entry point: sparkdiffusion/inference/wan2pt1_t2v_distilled_infer.py   (no image_path)
#                sparkdiffusion/inference/wan2pt1_i2v_distilled_infer.py   (image_path given)
#
#   bash scripts/inference/eval_student_2pt1_distilled.sh <ckpt_root> [output_dir] [num_steps] [bf16|fp8] [model_size] [topk] [image_path] [prompt]
#
# ---------------------------------------------------------------------------
...
#
# 1) 14B_rola, 4 steps, FP8 (480p 5s)
#    CUDA_VISIBLE_DEVICES=2 bash scripts/inference/eval_student_2pt1_distilled.sh \
#      pretrain_weights/Wan2.1-T2V-14B-Diffusers outputs/inference/wan21_14b_rola 4 fp8 14B_rola "" \
#      "A cat playing in the garden under the sun."
#
# 2) Dense 14B, BF16 baseline
#    CUDA_VISIBLE_DEVICES=2 bash scripts/inference/eval_student_2pt1_distilled.sh \
#      pretrain_weights/Wan2.1-T2V-14B-Diffusers outputs/inference/wan21_14b_bf16 4 bf16 14B "" \
#      "A cat playing in the garden under the sun."
#
# 3) 95% sparsity (top-k 0.05 instead of the default 0.1)
#    CUDA_VISIBLE_DEVICES=2 bash scripts/inference/eval_student_2pt1_distilled.sh \
#      pretrain_weights/Wan2.1-T2V-14B-Diffusers outputs/inference/wan21_14b_rola 4 fp8 14B_rola 0.05 "" \
#      "A cat playing in the garden under the sun."
#
# 4) 720p, one prompt
#    CUDA_VISIBLE_DEVICES=2 RESOLUTION=720p \
#      bash scripts/inference/eval_student_2pt1_distilled.sh \
#      pretrain_weights/Wan2.1-T2V-14B-Diffusers outputs/inference/wan21_720p 4 fp8 14B_rola "" \
#      "A cat playing in the garden under the sun."
#
# 5) I2V 720p 95% sparsity — 7th arg is the reference image
#    CUDA_VISIBLE_DEVICES=2 RESOLUTION=720p bash scripts/inference/eval_student_2pt1_distilled.sh \
#      pretrain_weights/Wan2.1-I2V-14B-480P-Diffusers \
#      outputs/inference/wan21_i2v_720p 4 fp8 14B_rola 0.05 examples/i2v_input_1.jpg \
#      "A person walks through a forest."
#
# 6) DCP checkpoint dir (converted to .pt once, then cached)
#    bash scripts/inference/eval_student_2pt1_distilled.sh output/job/checkpoints/iter_000010000/model $OUT/my_exp
#
# ---------------------------------------------------------------------------
...
#   - For Wan2.2 (dual high/low-noise students) use scripts/inference/eval_student_2pt2_distilled.sh.
# ============================================================================
set -euo pipefail
cd "$(dirname "$0")/../.."
export PYTHONPATH=.
...
USAGE="Usage: bash scripts/inference/eval_student_2pt1_distilled.sh <ckpt_root> [output_dir] [num_steps] [bf16|fp8] [model_size] [topk] [image_path] [prompt]"
CKPT_ROOT="${1:?$USAGE}"
...
if [ -n "$IMAGE_PATH" ]; then
    TASK="i2v"
    ENTRY="sparkdiffusion/inference/wan2pt1_i2v_distilled_infer.py"
    [ -f "$IMAGE_PATH" ] || { echo "image_path not found: $IMAGE_PATH" >&2; exit 2; }
else
    TASK="t2v"
    ENTRY="sparkdiffusion/inference/wan2pt1_t2v_distilled_infer.py"
fi
...
case "$MODEL_SIZE" in
    ...
    A14B*)
        echo "model_size '$MODEL_SIZE' is a Wan2.2 config; use scripts/inference/eval_student_2pt2_distilled.sh instead." >&2
        exit 2 ;;
    *)
...
if [ "${QUANT_ATTN:-0}" = "1" ] && [ -n "$QUANT_TYPE" ]; then
    echo "[eval_student_2pt1_distilled] note: QUANT_ATTN=1 is a no-op here; fp8 already covers FFN + attention Q/K/V/O."
fi

resolve_ckpt_root "$CKPT_ROOT" "eval_student_2pt1_distilled"
...
SAVE_PATH="$OUTPUT_DIR/${MODEL_SIZE}_${PRECISION}${TASK_TAG}_distilled${SUFFIX}.mp4"
...
[ "$TASK" = "i2v" ] && echo "[eval_student_2pt1_distilled] clip=$CLIP_ENCODER (fixed_resolution=${FIXED_RESOLUTION:-0})"
[ -n "$ROLA_TOPK" ] && echo "[eval_student_2pt1_distilled] rola_topk_ratio=$ROLA_TOPK (sparsity $SPARSITY)"
echo "[eval_student_2pt1_distilled] prompt=$PROMPT"
echo "[eval_student_2pt1_distilled] output=$SAVE_PATH"
echo "======================================================================"
...
echo "[eval_student_2pt1_distilled] exec: python $ENTRY"
python "$ENTRY" "${ARGS[@]}"
