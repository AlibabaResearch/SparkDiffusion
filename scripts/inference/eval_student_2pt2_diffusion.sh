#!/bin/bash
# ============================================================================
# Wan2.2 T2V single-case diffusion evaluation (UniPC + CFG, two MoE experts).
#   entry point: sparkdiffusion/inference/wan2pt2_t2v_diffusion_infer.py
#
#   bash scripts/inference/eval_student_2pt2_diffusion.sh <ckpt_root> [output_dir] [num_steps] [bf16|fp8] [model_size] [topk] [prompt]
#
...
# with non-distilled (teacher/base) high- and low-noise checkpoints.
#
# Experts are paged to the GPU one at a time at the boundary switch; both 14B
# experts resident at once OOMs on 32 GB.
#
# ---------------------------------------------------------------------------
...
#
# 1) A14B_rola, 8 steps, FP8 (480p 5s), same ckpt for both experts
#    CUDA_VISIBLE_DEVICES=2 bash scripts/inference/eval_student_2pt2_diffusion.sh \
#      pretrain_weights/Wan2.2-T2V-A14B-Diffusers outputs/inference/wan22_diff 8 fp8 A14B_rola "" \
#      "A cat playing in the garden under the sun."
#
# 2) Separate high/low-noise experts
#    CUDA_VISIBLE_DEVICES=2 CKPT_LOW=pretrain_weights/Wan2.2-T2V-A14B-Diffusers/transformer_2 \
#      bash scripts/inference/eval_student_2pt2_diffusion.sh \
#      pretrain_weights/Wan2.2-T2V-A14B-Diffusers outputs/inference/wan22_diff_real 8 fp8 A14B_rola "" \
#      "A cat playing in the garden under the sun."
#
# 3) 95% sparsity (top-k 0.05 instead of the default 0.1)
#    CUDA_VISIBLE_DEVICES=2 bash scripts/inference/eval_student_2pt2_diffusion.sh \
#      pretrain_weights/Wan2.2-T2V-A14B-Diffusers outputs/inference/wan22_diff 8 fp8 A14B_rola 0.05 \
#      "A cat playing in the garden under the sun."
#
# 4) One prompt at 720p
#    CUDA_VISIBLE_DEVICES=2 RESOLUTION=720p \
#      bash scripts/inference/eval_student_2pt2_diffusion.sh \
#      pretrain_weights/Wan2.2-T2V-A14B-Diffusers outputs/inference/wan22_diff_720p 8 fp8 A14B_rola "" \
#      "A cat playing in the garden under the sun."
#
# ---------------------------------------------------------------------------
...
#   - For Wan2.1 (single model) use scripts/inference/eval_student_2pt1_diffusion.sh.
# ============================================================================
set -euo pipefail
cd "$(dirname "$0")/../.."
export PYTHONPATH=.
...
ENTRY="sparkdiffusion/inference/wan2pt2_t2v_diffusion_infer.py"
...
USAGE="Usage: bash scripts/inference/eval_student_2pt2_diffusion.sh <ckpt_root> [output_dir] [num_steps] [bf16|fp8] [model_size] [topk] [prompt]"
CKPT_ROOT="${1:?$USAGE}"
...
case "$MODEL_SIZE" in
    ...
    1.3B*|14B*)
        echo "model_size '$MODEL_SIZE' is a Wan2.1 config; use scripts/inference/eval_student_2pt1_diffusion.sh instead." >&2
        exit 2 ;;
    *)
...
if [ "${QUANT_ATTN:-0}" = "1" ] && [ -n "$QUANT_TYPE" ]; then
    echo "[eval_student_2pt2_diffusion] note: QUANT_ATTN=1 is a no-op here; fp8 already covers FFN + attention Q/K/V/O."
fi

resolve_ckpt_root "$CKPT_ROOT" "eval_student_2pt2_diffusion"
DIT_PATH_LOW="${CKPT_LOW:-$DIT_PATH}"

mkdir -p "$OUTPUT_DIR"
SUFFIX=""
[ -n "$ROLA_TOPK" ] && SUFFIX="_topk${ROLA_TOPK}"
SAVE_PATH="$OUTPUT_DIR/${MODEL_SIZE}_${PRECISION}_diffusion${SUFFIX}.mp4"
...
[ -n "$ROLA_TOPK" ] && echo "[eval_student_2pt2_diffusion] rola_topk_ratio=$ROLA_TOPK (sparsity $SPARSITY)"
echo "[eval_student_2pt2_diffusion] prompt=$PROMPT"
echo "[eval_student_2pt2_diffusion] output=$SAVE_PATH"
echo "======================================================================"
...
echo "[eval_student_2pt2_diffusion] exec: python $ENTRY"
python "$ENTRY" "${ARGS[@]}"
