...
def launch(config: Config, args: argparse.Namespace) -> None:
    ...
    config.validate()
    # Inject trainer seed into dataloader for reproducible data ordering.
    # OmegaConf struct mode blocks new keys; temporarily disable it.
    from omegaconf import OmegaConf
    _dataloader_train_target = getattr(config.dataloader_train, "_target_", "") if config.dataloader_train is not None else ""
    _dataloader_val_target   = getattr(config.dataloader_val,   "_target_", "") if config.dataloader_val   is not None else ""
    if config.dataloader_train is not None and "create_dataloader" in str(_dataloader_train_target):
        OmegaConf.set_struct(config.dataloader_train, False)
        config.dataloader_train.seed = config.trainer.seed
        OmegaConf.set_struct(config.dataloader_train, True)
    if config.dataloader_val is not None and "create_dataloader" in str(_dataloader_val_target):
        OmegaConf.set_struct(config.dataloader_val, False)
        config.dataloader_val.seed = config.trainer.seed
        OmegaConf.set_struct(config.dataloader_val, True)
