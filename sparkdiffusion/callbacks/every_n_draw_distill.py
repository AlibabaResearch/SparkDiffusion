...
video_prompts = [
    ...
    "A boat sailing leisurely along the seine river with the eiffel tower in the background, van gogh style.",
]
...
class EveryNDrawSample_Distill(EveryN):
        ...
        if self.sample_fix:
            ...
            for prompt in video_prompts:
                ...
                self.kv_prompt_to_emb[prompt] = (
                    get_umt5_embedding(
                        checkpoint_path=model.config.text_encoder_path,
                        prompts=prompt,
                        tokenizer_path=getattr(model.config, "tokenizer_path", None),
    ...
    def every_n_impl(self, trainer, model, data_batch, output_batch, loss, iteration):
        # Only the EMA branch produces local artifacts now: a single fixed-prompt
        # generation set (concatenated grid + per-prompt individual mp4s).
        # The reg branch and the per-batch random-prompt sample() are skipped
        # entirely (no videos, no BatchInfo dump).
        if not self.is_ema:
            return
        if model.config.ema.enabled:
            ...
            # Fallback: sample with student net when EMA is disabled.
            from contextlib import nullcontext
            context = nullcontext
            tag = "reg"
        ...
        with context():
            ...
            # if self.is_sample and self.sample_fix:
            #     log.debug("entering, sample_fixed", rank0_only=False)
            #     self.sample_fixed(trainer, model, data_batch, output_batch, loss, iteration)
            #     log.debug("done, sample_fixed", rank0_only=False)
            if self.is_sample:
        ...
        if wandb.run:
            ...
            wandb.log(
                {"trainer/global_step": iteration, "sample_counter": sample_counter},
                step=iteration,
            )
    ...
    def sample(self, trainer, model, data_batch, output_batch, loss, iteration):
        ...
        to_show = []
        sample_student = model.generate_samples_viz(
            data_batch,
    ...
    def sample_fixed(self, trainer, model, data_batch, output_batch, loss, iteration):
        """
        Generate videos for the fixed prompt list.

        Saves two artifacts under ``self.local_dir`` (rank0 + dp0 only):
          1. Per-prompt individual mp4: ``0_{tag}_Sample_Iter{iter}_prompt{idx:02d}.mp4``
          2. Concatenated grid mp4    : ``0_{tag}_Sample_Iter{iter}.mp4``
        """
        ...
        save_individual = is_tp_cp_pp_rank0() and self.data_parallel_id == 0 and self.data_parallel_id < self.n_sample_to_save

        for idx, (prompt, text_emb) in enumerate(self.kv_prompt_to_emb.items()):
            ...
            # generate samples
            sample = model.generate_samples_viz(data_batch, seed=1)
            ...
            # Save the individual (non-concatenated) per-prompt video.
            if save_individual:
                single = (1.0 + video_cpu.clamp(-1, 1)) / 2.0  # [num_samples, c, t, h, w]
                # 1) Save grid version (all samples side-by-side along width).
                single_grid = rearrange(single, "b c t h w -> c t h (b w)")
                single_fp = f"{self.local_dir}/0_{tag}_Sample_Iter{iteration:09d}_prompt{idx:02d}"
                save_image_or_video(single_grid, single_fp)
                # # 2) Save each sample individually.
                # for s_idx in range(single.shape[0]):
                #     per_sample_fp = f"{self.local_dir}/0_{tag}_Sample_Iter{iteration:09d}_prompt{idx:02d}_s{s_idx:02d}"
                #     save_image_or_video(single[s_idx], per_sample_fp)  # [c, t, h, w]

            del sample, video, video_cpu

        # Save the concatenated grid version (all prompts stacked vertically).
