...
class WandbCallback(Callback):
    def __init__(self, logging_iter_multipler: int = 1, save_logging_iter_multipler: int = 1, save_s3: bool = False) -> None:
        ...
        # Per-closure loss records (lazy-init), e.g. "scm" / "dcm" / "dmd" / "critic".
        # Populated from output_batch["loss_<name>"] written by trainer_distillation.
        self.train_per_loss_log: Dict[str, _LossRecord] = {}
        # Gradient conflict metrics (lazy-init), e.g. "grad_cos_global" / "grad_norm_scm".
        self.train_grad_metrics_log: Dict[str, _LossRecord] = {}
    ...
    def on_training_step_end(
        ...
        if not skip_update_due_to_unstable_loss:
            ...
            # Per-closure loss records (e.g. loss_scm / loss_dcm / loss_dmd / loss_critic).
            # Trainer writes Python float into output_batch["loss_<name>"]; wrap it back to
            # a cuda tensor so existing _LossRecord.update / all_reduce path works as-is.
            for k, v in output_batch.items():
                if not (isinstance(k, str) and k.startswith("loss_")):
                    continue
                name = k[len("loss_"):]
                if not name:
                    continue
                if isinstance(v, torch.Tensor):
                    v_tensor = v.detach().float().to("cuda")
                else:
                    try:
                        v_tensor = torch.tensor(float(v), device="cuda", dtype=torch.float32)
                    except (TypeError, ValueError):
                        continue
                if torch.isnan(v_tensor) or torch.isinf(v_tensor):
                    continue
                if name not in self.train_per_loss_log:
                    self.train_per_loss_log[name] = _LossRecord()
                self.train_per_loss_log[name].update(v_tensor)

            # Gradient conflict metrics (e.g. grad_cos_global / grad_norm_scm / grad_cos_block_0).
            for k, v in output_batch.items():
                if not (isinstance(k, str) and (k.startswith("grad_cos_") or k.startswith("grad_norm_"))):
                    continue
                try:
                    v_tensor = torch.tensor(float(v), device="cuda", dtype=torch.float32)
                except (TypeError, ValueError):
                ...
                if torch.isnan(v_tensor) or torch.isinf(v_tensor):
                    continue
                if k not in self.train_grad_metrics_log:
                    self.train_grad_metrics_log[k] = _LossRecord()
                self.train_grad_metrics_log[k].update(v_tensor)
        else:
        ...
        if iteration % (self.config.trainer.logging_iter * self.logging_iter_multiplier) == 0:
            ...
            # Aggregate per-closure stats. Use a sorted, stable key list so all ranks
            # call all_reduce in the exact same order (closures are deterministic across ranks).
            per_loss_stats: Dict[str, Dict[str, float]] = {}
            for name in sorted(self.train_per_loss_log.keys()):
                per_loss_stats[name] = self.train_per_loss_log[name].get_stats()

            # Aggregate gradient conflict metrics (sorted for all_reduce consistency).
            grad_metrics_stats: Dict[str, Dict[str, float]] = {}
            for name in sorted(self.train_grad_metrics_log.keys()):
                grad_metrics_stats[name] = self.train_grad_metrics_log[name].get_stats()
            ...
            if distributed.is_rank0():
                ...
                # Add per-closure loss statistics (loss_scm / loss_dmd / loss_critic / ...)
                for name, stats in per_loss_stats.items():
                    info[f"train{self.wandb_extra_tag}/loss_{name}"] = stats["loss"]

                # Add gradient conflict metrics (grad_cos_global / grad_norm_scm / ...)
                for name, stats in grad_metrics_stats.items():
                    info[f"train{self.wandb_extra_tag}/{name}"] = stats["loss"]
