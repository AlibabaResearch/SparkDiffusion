...
from sparkdiffusion.checkpointers.dcp import AsyncMode, DefaultLoadPlanner, DefaultSavePlanner
from sparkdiffusion.checkpointers.dcp import DistributedCheckpointer
from sparkdiffusion.checkpointers.dcp import ModelWrapper, OptimizerWrapper
...
class DistributedCheckpointer_Distill(DistributedCheckpointer):
    ...
    def save_state_dict_worker(self, to_save_dict: Dict[str, Tuple[Any, str]], checkpoint_file: str) -> None:
        # dcp.save gathers save plans via NCCL, whose buffers come from raw cudaMalloc and cannot
        # reuse the caching allocator's reserved pool. At high resolution that pool leaves almost no
        # free VRAM, so release it here or the gather fails with "unhandled cuda error".
        torch.cuda.empty_cache()
    ...
    def save(
        ...
        # ── Auto-export DCP → .pth on rank 0 ──
        import torch.distributed as dist
        if not dist.is_initialized() or dist.get_rank() == 0:
            dcp_model_dir = os.path.join(self.save_dirname, f"iter_{iteration:09}", "model")
            pth_save_path = os.path.join(self.save_dirname, f"iter_{iteration:09}", "exported.pth")
            if os.path.isdir(dcp_model_dir):
                try:
                    from torch.distributed.checkpoint import FileSystemReader
                    from torch.distributed.checkpoint.default_planner import _EmptyStateDictLoadPlanner
                    from torch.distributed.checkpoint.state_dict_loader import _load_state_dict
                    sd = {}
                    _load_state_dict(sd, storage_reader=FileSystemReader(dcp_model_dir),
                    ...
                    # Joint finetuning stores the two experts as net_high/net_low.
                    # Keep exported.pth as the canonical high-expert filename for
                    # compatibility with existing inference entrypoints.
                    high_sd = _collect("net_high.")
                    if not high_sd:
                        high_sd = _collect("net_ema.")
                    if not high_sd:
                        high_sd = _collect("net.")
                    low_sd = _collect("net_low.")

                    if high_sd:
                        torch.save(high_sd, pth_save_path)
                        log.info(f"Auto-exported high .pth to {pth_save_path} ({len(high_sd)} keys)")
                    if low_sd:
                        low_save_path = os.path.join(self.save_dirname, f"iter_{iteration:09}", "exported_low.pth")
                        torch.save(low_sd, low_save_path)
                        log.info(f"Auto-exported low .pth to {low_save_path} ({len(low_sd)} keys)")
                except Exception as e:
                    log.warning(f"Auto-export .pth failed (non-fatal): {e}")
