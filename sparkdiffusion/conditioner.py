...
from sparkdiffusion.utils.misc import batch_mul
...
class TextCondition(BaseCondition):
    ...
    # I2V (image-to-video) condition: first-frame latent (16) + mask (4) concatenated into 20ch,
    # built by the model's get_data_and_condition. None for T2V, where net forward skips the concat.
    y_B_C_T_H_W: Optional[torch.Tensor] = None
    # Wan2.1 I2V CLIP image features [B,257,1280], injected into cross-attention after the DiT's
    # img_emb projection. The field name must match the wan2pt1 net forward argument name
    # (to_dict expands into kwargs).
    # Only used by Wan2.1 I2V (2.2 I2V has no CLIP branch; this field is None and forward falls
    # back through **kwargs).
