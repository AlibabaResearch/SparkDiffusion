...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Few-step distilled inference script for Wan2.1 T2V")
...
if __name__ == "__main__":
    ...
    all_text_embs = get_umt5_embedding(checkpoint_path=args.text_encoder_path, prompts=prompts, tokenizer_path=args.tokenizer_path).to(dtype=torch.bfloat16).cuda()
    clear_umt5_memory()


    # --- Precompute schedule (shared across prompts), rf domain ---
    # Legacy TrigFlow knots pre-converted via rf = sin(t)/(cos(t)+sin(t)).
    MID_T_BY_NSTEPS = {
        1: [],
        2: [0.933781],
        3: [0.933781, 0.852895],
