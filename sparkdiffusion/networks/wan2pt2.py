...
class WanModel(nn.Module):
    ...
    def __init__(
        ...
        # Inference-only: enable the fused Triton RoLa attention kernels.
        use_fused_inference: bool = False,
        # Plugin: explicitly select a sparse attention variant from the registry (takes
        # precedence over the use_rola_attn bool switch). When None, the original logic
        # below runs unchanged. attn_variant_kwargs is forwarded to the class constructor.
        ...
        if attn_variant is not None:
            # Plugin path: fetch the custom sparse attention from the registry (does not affect the legacy use_rola_attn logic below)
        ...
        self.blocks = nn.ModuleList(
            [WanAttentionBlock(cross_attn_type, dim, ffn_dim, num_heads, qk_norm, cross_attn_norm, eps,
                               attn_class=_attn_class, attn_kwargs=_attn_kwargs) for _ in range(num_layers)]
        )
        # Inference-only: let each self-attention pick its fused Triton path.
