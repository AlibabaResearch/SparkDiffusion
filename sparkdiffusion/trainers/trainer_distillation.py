...
class ImaginaireTrainer_Distill:
    ...
    def training_step(
        ...
        for name, closure, is_last_closure in closures_iter:
            ...
            loss_total = loss_total + float(loss_i.detach().mean())
            # Per-closure loss for separate logging (e.g. loss_scm / loss_dmd / loss_critic).
            # Skip if the model already wrote loss_<name> with a more granular value
            # (e.g. dmd closure writes pure DMD here, leaving loss_div as a separate key).
            key = f"loss_{name}"
            if key not in output_batch:
                output_batch[key] = float(loss_i.detach().sum())
