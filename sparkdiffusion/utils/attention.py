...
def attention(
    ...
    # Let Inductor own SDPA dispatch while tracing. Querying CUDA capability
    # and entering a Python backend-selection context would otherwise create
    # a graph break at every self/cross-attention call.
    if torch.compiler.is_compiling() and not deterministic:
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
