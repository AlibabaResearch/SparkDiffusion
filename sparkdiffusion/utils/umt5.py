    ...
    if distributed.is_rank0():
        if os.path.isfile(ckpt_path):
            try:
                ...
                log.warning(
                    f"weights_only load failed for {ckpt_path}; falling back "
                    "to full unpickling without mmap (trusted file assumed)"
                )
                ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False, mmap=False)
        else:
            ckpt = easy_io.load(
                ckpt_path,
                map_location="cuda",
...
class UMT5EncoderModel:
    def __init__(
        ...
        checkpoint_path="pretrain_weights/Wan2.1-T2V-1.3B/Wan2.1-T2V-1.3B/models_t5_umt5-xxl-enc-bf16.pth",
...
def get_umt5_embedding(
    ...
    if t5_encoder is None:
        ...
        t5_encoder = UMT5EncoderModel(text_len=max_length, device=device, checkpoint_path=checkpoint_path, **kwargs)
